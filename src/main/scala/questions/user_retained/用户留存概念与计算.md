...
-- 方法1
select t0.active_date                   as dt,
       round(count(distinct(case when TIMESTAMPDIFF(day, t0.active_date, t1.active_date) = 1 then t1.uid else null end)) / count(distinct t0.uid), 2) as rate
from (select uid
           , min(date(in_time)) active_date
      from tb_user_log
      group by uid) t0 -- 新增用户
         left join (select uid,
                    ...
                    from tb_user_log) t1
                   on t0.active_date <= t1.active_date
                       and t0.uid = t1.uid
where substring(t0.active_date, 1, 7) = '2021-11'
group by t0.active_date
order by dt;
